...
import com.cyanrocks.ui.listen.LlmAccessTokenService;
...
public class LlmCardManager {
    ...
    public void streamCallWithCallback(String query, GbiCardParamBO cardParamBO, Function<GbiCardParamBO, String> streamingFunction, Function<GbiCardParamBO, String> updateFunction, String dingId) throws Exception {
        Map<String, String> result = milvusUtils.semanticSearch2(query,"pdf_markdown_new",dingId);
        //流式输出文本结果
        ...
        if (StringUtils.isNotEmpty(title)){
            ...
            String[] sources = result.get("source").split(",");
            for (int i = 0; i < titles.length; i++) {
                JSONObject resource = new JSONObject();
                resource.put("name", titles[i]);
                resource.put("url", "https://api.peidigroup.cn/ai/common/download?objectName="+sources[i]);
